/**
 * pages/BillPage.jsx
 * Shareable bill page — anyone with the link can view and pay their share.
 */
...
function SplitRow({ split }) {
  return (
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div>
          ...
          {split.paid && split.tx_hash && (
            <a href={`${EXPLORER}/tx/${split.tx_hash}`} target="_blank" rel="noreferrer"
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <span style={{ fontFamily:'var(--font-display)', fontWeight:700, color:'var(--text-100)', fontSize:'0.9rem' }}>
...
export default function BillPage({ user, account, onLogin, isLoading: walletLoading, onDeduct, onAdd }) {
  ...
  const handlePay = async () => {
    ...
    // Demo: use a placeholder address
    ...
    const hash = await send({
      ...
      toUsername: bill.creator_username,
    })
    if (!hash) return
      ...
      <div style={{ minHeight:'100dvh', paddingBottom:80, position:'relative', zIndex:1 }}>
        <div className="container" style={{ paddingTop:40 }}>
          ...
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
            transition={{ duration:0.5 }} className="card card-glow" style={{ padding:'28px 24px', marginBottom:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
              ...
              <span className={settled ? 'badge badge-green' : 'badge badge-orange'}>
          ...
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}
            transition={{ delay:0.1 }} className="card" style={{ padding:'20px', marginBottom:16 }}>
          ...
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }}>
            ...
            {!user && (
              <div className="card card-glow" style={{ padding:'28px 24px', textAlign:'center' }}>
                ...
                <p style={{ fontSize:'0.88rem', marginBottom:24 }}>Sign in to see your share and pay gaslessly.</p>
                <button className="btn btn-primary" onClick={onLogin} disabled={walletLoading}
            ...
            {mySplit && mySplit.paid && (
              <div className="card" style={{ padding:'28px 24px', textAlign:'center', background:'var(--green-glow)', borderColor:'rgba(16,185,129,0.2)' }}>
                ...
                {mySplit.tx_hash && (
                  <a href={`${EXPLORER}/tx/${mySplit.tx_hash}`} target="_blank" rel="noreferrer"
                    className="btn btn-ghost" style={{ fontSize:'0.8rem', padding:'9px 18px' }}>
                    View on Starkscan ↗
                  </a>
                )}
              </div>
            ...
            {userOwes && (
              <div className="card card-glow" style={{ padding:'28px 24px' }}>
                ...
                <p style={{ fontSize:'0.88rem', marginBottom:20 }}>
                  You owe{' '}
                  <span style={{ color:'var(--orange-2)', fontFamily:'var(--font-display)', fontWeight:700 }}>
                ...
                <button className="btn btn-primary" onClick={handlePay}
                  disabled={sending || !account} style={{ width:'100%', padding:'16px', fontSize:'0.95rem' }}>
                  {sending
